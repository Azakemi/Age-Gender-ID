...
while True:
    ...
    if len(bboxs) > 0:
        bbox = bboxs[0]
        if last_bbox is None or bbox != last_bbox:
            tally = defaultdict(lambda: defaultdict(int))
            last_bbox = bbox
        ...
        most_frequent_gender = max(tally['gender'], key=tally['gender'].get)
        most_frequent_age = max(tally['age'], key=tally['age'].get)

        label = "{},{}".format(most_frequent_gender, most_frequent_age)
